...
class BivariateColorMap:
    ...
    def sorted_labels(self, reverse: bool = True) -> list[str]:
    ...
    def plot_color_square(self, show_labels=True) -> plt.Axes:
        ...
        labels = np.array(self.sorted_labels()).reshape(self.n_rows, self.n_rows)
    ...
    def cmap(self) -> dict[str:str]:
        ...
        return dict(zip(self.sorted_labels(reverse=False), self.colors))
